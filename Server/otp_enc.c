 ...
 * plaintext and key file, send the contents to the server for encryption
...
#define READMAX 64
#define ID 42               /* this is the ID we send to the server for identification*/
 ...
 * The first file should be a plaintext filename and the second file the key file
 * The port should be the port that the otp_dec_d server is listening on.
 ...
 * Then sends the plaintext string and keytext line.
 * Finally waits for the response from the server which should be the ciphertext
 * outputs this to STDOUT. 
 * Preconditions: passed 2 valid filenames and a valid port
 * Postconditions: ciphertext output to STDOUT
 * *************************************/

int main(int argc, char* argv[]){

    int socketFD, inFileFD, keyFileFD, 
        inFileLen, keyFileLen, id, response,
        bytesSent, bytesRead;
    ...
    inFileFD = openFile(argv[1]); 
    keyFileFD = openFile(argv[2]); 
    /* compare the file lengths, if the key file is not long enough we bail here */
    inFileLen = getFileLength(inFileFD);
    keyFileLen = getFileLength(keyFileFD);
    compareFileLen(inFileLen, keyFileLen, argv[2]); 

    /* read in the plaintext and verify that all the characters are acceptable */
    plaintext = readFile(inFileLen, inFileFD);
    verifyLine(plaintext, argv[1]);

    /* we don't validate the keytext because we expect it to be generated from keygen*/
    /* that will only output correct characters to a file */
    keytext = readFile(inFileLen, keyFileFD);

    socketFD = initServer(argv[3], &sevAdd);

    /* set a timeout option so that our client drops any recv() after 30 seconds */
    /* this will set the errno to EAGAIN or EWOULDBLOCK and we'll check for this */
    /* in the while loops where we are receving bytes from the server */
    tv.tv_sec = 30; 
    tv.tv_usec = 0;
    ...
    /* if the response does not match our id we connected to the wrong server */
    if(response != id){
        free(plaintext);
        free(keytext);
        ...
        /* send our plaintext */
        sendText(socketFD, plaintext);
        /* wait for the server to respond that it has received all of the plaintext */
        recv(socketFD, &response, sizeof(int), 0); 
        /* send our keytext when ready */
        sendText(socketFD, keytext);
        /* finally we wait for the encrypted message to come back here */
...
void getErrMsg(int socketFD, int portNum){
    ...
    /* and exit with 2 */
    fprintf(stderr, "Error: otp_enc may not connect to %s on port %d\n", msg, portNum);
    free(msg);
    close(socketFD);
    exit(2);
