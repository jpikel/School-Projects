...
def improved_nearest(FILE_NAME):
    """This is an improvement over the nearest_dblimp.py such that some of the for
    loops that were previously separate are now combined, so that it iterates through
    the list fewer total times.  Gave some improvement to the processing speed
    """
...
#between city pairs for the original distance matrix and store it in a 2-D array
    ...
    distset = []
    #compute the distance matrix for all cities and find the two cities that are closest
    #to each other
...
#others
    ...
    for city_1 in list_of_cities: 
        if distset[city_1[0]] < min_distset:
...
#5-Calculate the new distance matrix from the combination of the two criteria*/ 
    ...
    opt_distance = nearest_dist         #set the opt_distance to the two nearest cities
    ...
    while len(sol_of_cities) < len(list_of_cities):         #check the current city to
            ...
            if nearest_city[3] != True and nearest_city_2[3] != True:
                if nearest_dist < nearest_dist_2:
                    sol_of_cities.append(nearest_city[0])
                    dx = city_1[1] - nearest_city[1]
                    dy = city_1[2] - nearest_city[2]
                    opt_distance += int(round(math.sqrt((dx*dx) + (dy*dy))))
                    nearest_city[3] = True
                    city_1 = nearest_city
                else:
                    sol_of_cities.insert(0, nearest_city_2[0])
                    dx_2 = city_3[1] - nearest_city_2[1]
                    dy_2 = city_3[2] - nearest_city_2[2]
                    opt_distance += int(round(math.sqrt((dx_2*dx_2) + (dy_2*dy_2))))
